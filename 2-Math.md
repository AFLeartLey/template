...
namespace BerlekampMassey {
    ...
    V mul(const V&a, const V& b, const V& m, int k) {
        V r; r.resize(2 * k - 1);
        for(int i = 0; i < k; ++ i) for(int j = 0; j < k; ++ j) up(r[i + j], a[i] * b[j]);
        for(int i = k - 2; ~i; -- i) {
            for(int j = 0; j < k; ++ j) up(r[i + j], r[i + k] * m[j]);
            r.pop_back();
        }
        return r;
    }
    ...
    V BM(const V& x) {
        ...
        for(int n = 0; n < x.size(); ++ n) {
            ...
            for(int _ = C.size(); _ < B.size() + m; ++ _) C.push_back(0);
            for(int i = 0; i < B.size(); ++ i) up(C[i + m], c * B[i]);
            if (2 * L > n) { ++m; continue; }
...
namespace lp {
    ...
    void pivot(int l, int e) {
        b[l] /= a[l][e];
        for(int j = 0; j < n; ++ j) if (j != e) a[l][j] /= a[l][e];
        a[l][e] = 1 / a[l][e];
        ...
        v += c[e] * b[l];
        for(int j = 0; j < n; ++ j) if (j != e) c[j] -= c[e] * a[l][j];
        c[e] = -c[e] * a[l][e];
    }
    double simplex() {
        while (1) {
            v = 0;
            int e = -1, l = -1;
            for(int i = 0; i < n; ++ i) if (c[i] > eps) { e = i; break; }
            if (e == -1) return v;
            double t = INF;
            for(int i = 0; i < m; ++ i)
