...
void init() {
    for(auto x = 0, _##x = highbit(n) ; x <= _##x; ++x)
    for(auto y = 0, _##y = highbit(m) ; y <= _##y; ++y)
...
namespace kd {
    ...
    void init() {
        ...
        for(int i = 0;i < K; ++ i) null->l[i] = inf, null->r[i] = -inf;
        null->sum = null->val = 0;
        null->sz = 0;
    }
    ...
    inline bool inside(int p[], int q[], int l[], int r[]) {
        for(int i = 0;i < K; ++ i)if (r[i] < q[i] || p[i] < l[i]) return false;
        return true;
    }
    LL query(P* o, int l[], int r[]) {
        if (o == null) return 0;
        for(int i = 0; i < K; ++ i) if (o->r[i] < l[i] || r[i] < o->l[i]) return 0;
...
void go(int now, int l, int r) {
    if (now == 0) {
        ...
        int m = (l + r) / 2;
        go(now, l, m);
        for(int i = l; i < m; ++ i) a[now][i]->d[now] = 0;
        for(int i = m; i < r; ++ i) a[now][i]->d[now] = 1;
        ...
        if (now < k - 2) {
            ...
            for(int i = l; i < r; ++ i){
                ...
                int cnt = 0;
                for(int j = 0; j <= now; ++ j) cnt += a[now][i]->d[j];
                if (cnt == 0) {
                    sum += a[now][i]->f;
                } else if (cnt == now + 1) {
...
int main() {
    for(int i = 0;i < 5; ++ i) s.push_back(i); // 0 1 2 3 4
